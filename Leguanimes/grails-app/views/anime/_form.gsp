...
<div class="fieldcontain ${hasErrors(bean: animeInstance, field: 'removedAt', 'error')} ">
	<label for="removedAt">
		<g:message code="anime.removedAt.label" default="Removed At" />
		
	</label>
	<g:datePicker name="removedAt" precision="day"  value="${animeInstance?.removedAt}" default="none" noSelection="['': '']" />

</div>

<div class="fieldcontain ${hasErrors(bean: animeInstance, field: 'createdBy', 'error')} ">
	<label for="createdBy">
		<g:message code="anime.createdBy.label" default="Created By" />
		
	</label>
	<g:select id="createdBy" name="createdBy.id" from="${entities.User.list()}" optionKey="id" value="${animeInstance?.createdBy?.id}" class="many-to-one" noSelection="['null': '']"/>
