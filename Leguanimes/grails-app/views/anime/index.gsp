		...
		<div id="list-anime" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="removedAt" title="${message(code: 'anime.removedAt.label', default: 'Removed At')}" />
				...
				<g:each in="${animeInstanceList}" status="i" var="animeInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${animeInstance.id}">${fieldValue(bean: animeInstance, field: "removedAt")}</g:link></td>
					
						<td>${fieldValue(bean: animeInstance, field: "createdBy")}</td>
