		...
		<div id="list-ending" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="removedAt" title="${message(code: 'ending.removedAt.label', default: 'Removed At')}" />
				...
				<g:each in="${endingInstanceList}" status="i" var="endingInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${endingInstance.id}">${fieldValue(bean: endingInstance, field: "removedAt")}</g:link></td>
					
						<td>${fieldValue(bean: endingInstance, field: "createdBy")}</td>
					
						<td>${fieldValue(bean: endingInstance, field: "anime")}</td>
