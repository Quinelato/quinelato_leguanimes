...
<div class="fieldcontain ${hasErrors(bean: endingInstance, field: 'removedAt', 'error')} ">
	<label for="removedAt">
		<g:message code="ending.removedAt.label" default="Removed At" />
		
	</label>
	<g:datePicker name="removedAt" precision="day"  value="${endingInstance?.removedAt}" default="none" noSelection="['': '']" />

</div>

<div class="fieldcontain ${hasErrors(bean: endingInstance, field: 'createdBy', 'error')} ">
	<label for="createdBy">
		<g:message code="ending.createdBy.label" default="Created By" />
		
	</label>
	<g:select id="createdBy" name="createdBy.id" from="${entities.User.list()}" optionKey="id" value="${endingInstance?.createdBy?.id}" class="many-to-one" noSelection="['null': '']"/>
