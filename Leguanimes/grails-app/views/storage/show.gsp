...
<g:set var="entityName"
	value="${message(code: 'storage.label', default: 'Storage')}" />
<title><g:message code="default.show.label" args="[entityName]" /></title>
</head>
<body>
	<a href="#show-storage" class="skip" tabindex="-1"><g:message
		...
		<ol class="property-list storage">
			...
			<g:if test="${storageInstance?.name}">
				<li class="fieldcontain"><span id="name-label"
					class="property-label"><g:message code="storage.name.label"
							default="Name" /></span> <span class="property-value"
					aria-labelledby="name-label"><g:fieldValue
							bean="${storageInstance}" field="name" /></span></li>
			</g:if>

			<g:if test="${storageInstance?.description}">
				<li class="fieldcontain"><span id="description-label"
					class="property-label"><g:message
							code="storage.description.label" default="Description" /></span> <span
					class="property-value" aria-labelledby="description-label"><g:fieldValue
							bean="${storageInstance}" field="description" /></span></li>
			</g:if>
			
			<g:if test="${storageInstance?.number}">
				<li class="fieldcontain"><span id="number-label"
					class="property-label"><g:message
							code="storage.number.label" default="Number" /></span> <span
					class="property-value" aria-labelledby="number-label"><g:fieldValue
							bean="${storageInstance}" field="number" /></span></li>
			</g:if>

			<g:if test="${storageInstance?.burnDate}">
				<li class="fieldcontain"><span id="burnDate-label"
					class="property-label"><g:message
							code="storage.burnDate.label" default="Burn Date" /></span> <span
					class="property-value" aria-labelledby="burnDate-label"><g:formatDate
							date="${storageInstance?.burnDate}" /></span></li>
			</g:if>
			
			<g:if test="${storageInstance?.episodes}">
				<li class="fieldcontain"><span id="episodes-label"
					class="property-label"><g:message
							code="storage.episodes.label" default="Episodes" /></span> <g:each
						in="${storageInstance.episodes}" var="e">
			...
			<g:if test="${storageInstance?.active}">
				<li class="fieldcontain"><span id="active-label"
					class="property-label"><g:message
							code="storage.active.label" default="Active" /></span> <span
					class="property-value" aria-labelledby="active-label"><g:formatBoolean
							boolean="${storageInstance?.active}" /></span></li>
			</g:if>

		</ol>
		<g:form url="[resource:storageInstance, action:'delete']"
			method="DELETE">
			<fieldset class="buttons">
				<g:link class="edit" action="edit" resource="${storageInstance}">
